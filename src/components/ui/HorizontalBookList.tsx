...
const BookCard: React.FC<BookCardProps> = memo(
  ({ item, index, scrollX, animationType, cardLayout, snapInterval, onPress }) => {
    ...
    const animatedStyle = useAnimatedStyle(() => {
      ...
      if (animationType === "scale") {
        return {
          transform: [{ scale: interpolate(progress, [-1, 0, 1], [0.9, 1, 0.9]) }],
        };
      }
    ...
    const rootStyle = isHorizontal
      ? styles.cardRootHorizontal
      : isFeatured
        ? styles.cardRootFeatured
        : isCompact
          ? styles.cardRootCompact
          : styles.cardRootStandard;
      ...
      <Pressable onPress={handlePress} style={cardStyles}>
        <Animated.View style={[rootStyle, animatedStyle]}>
          ...
          <View style={isCompact ? styles.compactInfoWrap : styles.standardInfoWrap}>
            <Text style={styles.titleText} numberOfLines={isCompact ? 1 : 2}>
              {item.title}
            </Text>
            <Text style={styles.authorText} numberOfLines={1}>
              {item.author}
...
const HorizontalBookList: React.FC<HorizontalBookListProps> = memo(
  ({ title, books, animationType = "standard", cardLayout = "standard", onBookPress, onSeeAllPress, loop = false }) => {
