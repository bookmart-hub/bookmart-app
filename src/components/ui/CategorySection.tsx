...
const CategoryCard = memo(
  ({ item, index, scrollX, onPress }: CategoryCardProps) => {
    ...
    const animatedStyle = useAnimatedStyle(() => {
      ...
      const scale = interpolate(
        scrollX.value,
        [centerPosition - SNAP_SIZE, centerPosition, centerPosition + SNAP_SIZE],
        [0.85, 1.18, 0.85],
        Extrapolation.CLAMP
      );
    ...
    const titleStyle = useAnimatedStyle(() => {
      ...
      const opacity = interpolate(
        scrollX.value,
        [centerPosition - SNAP_SIZE / 2, centerPosition, centerPosition + SNAP_SIZE / 2],
        [0, 1, 0],
        Extrapolation.CLAMP
      );
    ...
    return (
      <TouchableOpacity activeOpacity={0.8} style={styles.itemWrapper} onPress={handlePress}>
  ...
  (prevProps, nextProps) => {
    return (
      prevProps.item.uniqueId === nextProps.item.uniqueId &&
...
const CategorySection = memo(({ categories = CATEGORIES_LIST }: CategorySectionProps) => {
  ...
  const middleIndex = Math.floor(LOOP_COPIES / 2) * categories.length;
  ...
  useEffect(() => {
    requestAnimationFrame(() => {
      ...
      scrollX.value = middleIndex * SNAP_SIZE;
    });
  }, [middleIndex]);
  ...
  const renderItem = useCallback(
    ({ item, index }: { item: ExtendedCategoryItem; index: number }) => (
      <CategoryCard item={item} index={index} scrollX={scrollX} onPress={handlePress} />
    ),
    [handlePress, scrollX]
  );
